 ...
 * a) We suppose that a list contains elements that are lists themselves. The objective is to sort the elements of the
 ...
 * the elements according to their length frequency; i.e. in the default, sorting is done ascendingly, lists with rare
...
object LSort {
  def lsort[A](ls: List[List[A]]): List[List[A]] = ls.sortBy(e => e.length)

  def lsortFreq[A](ls: List[List[A]]): List[List[A]] = ls.groupBy(_.length).toList.sortBy(_._2.length).flatMap(_._2)
